...
func _ready():
	...
	tween.connect("tween_all_completed", self, "_on_fade_in_done")
	tween.start()
	
func _on_fade_in_done():
	yield(get_tree().create_timer(_WAIT_SECONDS), "timeout")
	var tween = Tween.new()
	add_child(tween)
	tween.interpolate_property(_logo, "self_modulate", Color(1, 1, 1, 1), Color(1, 1, 1, 0), _FADE_SECONDS)
	tween.connect("tween_all_completed", self, "_on_fade_out_done")
	tween.start()
